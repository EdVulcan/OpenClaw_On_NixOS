export const observerClientEngineeringVerificationRenderersScript = `function renderEngineeringVerificationEvidence(data) {
  ...
  engineeringVerificationJson.textContent = [
    ...
    ...evidence.slice(0, 8).map((item) => {
      ...
      return \`\${item.ok ? "pass" : "fail"} task=\${item.taskId ?? "none"} command=\${item.commandShape?.command ?? "unknown"} cwd=\${item.commandShape?.cwd ?? "unknown"} exit=\${item.result?.exitCode ?? "n/a"} timeout=\${Boolean(item.result?.timedOut)} attached=\${Boolean(item.attachment?.attachedToTaskCompletion)} failedChecks=\${failed}\`;
    }),
    "",
