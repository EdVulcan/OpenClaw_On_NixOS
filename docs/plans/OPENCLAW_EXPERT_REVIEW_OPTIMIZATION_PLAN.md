...
| Event Hub audit-log memory | Development audit queries and summaries read and split the entire JSONL file; the default `.artifacts` log reached hundreds of MiB and could create a large transient heap spike. | Complete | `audit-log-store.mjs` uses reverse fixed-block tail reads, streaming cached summaries, serialized 64 MiB rotation, eight retained segments, and stable file-handle snapshots. 11 focused tests, event-audit, body-config, and the full workspace pass. The 438 MiB log measured about 41 ms/7 MiB heap for a 200-event tail, 1.9 s/20 MiB heap for the first summary, and 0 ms for a cached repeat. |
