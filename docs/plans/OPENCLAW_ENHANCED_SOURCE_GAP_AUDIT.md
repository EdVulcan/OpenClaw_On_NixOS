...
- Native engineering loop operator controls:
  ...
  operator-selected edit/write/verification inputs.
...
| Operator-facing UI refinements | partially absorbed | Observer UI has been decoupled into panels/refreshers/renderers and now exposes parameterized engineering loop controls for edit/write/verification approval tasks, but enhanced chat/tool-card styling is not migrated. | Keep product-native controls and improve loop-state guidance. Avoid wholesale CSS import. | Level 1 now, Level 2 when work-view is active |
