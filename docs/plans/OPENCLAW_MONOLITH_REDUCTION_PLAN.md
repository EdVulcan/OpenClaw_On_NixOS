...
| P1 | `services/openclaw-core/src/plan-builder.mjs` | Plugin plans, runtime activation, long-term memory, cloud live-provider wiring, body evidence append/readiness read models, task creation, approval creation, event publishing, and phase evidence builders still share one builder. The capability registry/invocation slice and systemd/body task-shell builders are now extracted. | Next safe boundaries are long-term-memory builders, cloud-consciousness task/materialization lanes, and the separate body evidence append/readiness read-model lane; keep the returned plan-builder method names stable while moving one cohesive family at a time. |
