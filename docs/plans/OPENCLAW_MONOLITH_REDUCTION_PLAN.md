...
| P1 | `services/openclaw-system-sense/src/server.mjs` | Bounded filesystem operations, command/process operations, read-only systemd inventory/dependency mapping, health/body-governance read models, body-evidence timeline/ledger route-review builders, and the next systemd repair planning chain are extracted into focused modules. The server still owns service discovery, generic systemd repair plan/task/execution helpers, body/system HTTP route dispatch, and broad endpoint registration. | Continue extracting system-sense domain modules behind stable endpoints: generic systemd repair task/execution helpers, then route registration groups for already-extracted system/body evidence surfaces. |
...
7. Continue splitting remaining `openclaw-system-sense` generic repair
   task/execution helpers and route registration groups, then return to the highest-coupling
