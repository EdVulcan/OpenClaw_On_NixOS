...
| P0 | `services/openclaw-core/src/task-executor.mjs` | Execution dispatch still imports many delegated `is...Task`/`execute...Task` pairs, but task matching uses a `NON_RECOVERABLE_TASK_HANDLERS` registry instead of an ordered if/else branch chain, and the system/body execution lane now lives in `task-executor-system-body-handlers.mjs`. Remaining coupling is native/search-web deferred handlers, generic work-view execution, capability invocation, and broad cloud delegated handler wiring. | Continue handler group extraction only for cohesive lanes that carry their predicates, private helpers, and registry entries together; next likely slice is native/search-web deferrals or generic work-view/capability helpers, not individual functions. |
