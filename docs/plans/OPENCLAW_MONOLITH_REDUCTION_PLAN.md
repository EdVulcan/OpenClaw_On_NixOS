...
| P1 | `services/openclaw-core/src/plugin-review.mjs` | Workspace/source intake, Plugin SDK contract/source/native-contract, and manifest/capability planning lanes are now extracted into focused modules. Remaining coupling still bundles native adapter status, search-web runtime activation/provider sandbox task creation, and workspace tool catalog/prompt/semantic/symbol/edit-target review in one composition surface. | Continue splitting review surfaces into search-web runtime and workspace semantic-index/tool-catalog modules while keeping public plugin review methods stable. |
