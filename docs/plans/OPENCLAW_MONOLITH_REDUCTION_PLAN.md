...
| P1 | `services/openclaw-core/src/plugin-review.mjs` | Workspace/source intake is now extracted into focused discovery, source-command, and migration modules. Remaining coupling still bundles plugin SDK contract/source review, native adapter checks, manifest/capability planning, search-web runtime activation, provider sandbox task creation, and workspace semantic/symbol/edit-target review in one composition surface. | Continue splitting review surfaces into SDK contract/source, manifest/capability, search-web runtime, and workspace semantic-index modules while keeping public plugin review methods stable. |
