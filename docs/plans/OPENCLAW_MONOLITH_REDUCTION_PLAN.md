...
| P1 | `services/openclaw-core/src/plugin-review.mjs` | Workspace/source intake, Plugin SDK contract/source/native-contract, manifest/capability planning, search-web read-model planning, and search-web task materialization lanes are now extracted into focused modules. Remaining coupling still bundles native adapter status with workspace tool catalog/prompt/semantic/symbol/edit-target review in one composition surface. | Continue splitting review surfaces into native adapter status and workspace semantic-index/tool-catalog modules while keeping public plugin review methods stable. |
