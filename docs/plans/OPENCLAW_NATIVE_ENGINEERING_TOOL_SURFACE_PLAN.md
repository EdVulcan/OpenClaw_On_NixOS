...
long-lived LSP process pool and JSON-RPC request handling; `lsp_evidence`
contract, availability evidence, lifecycle readiness draft, approval-gated
binary gate, bounded process supervision probe, and lifecycle state readback are
absorbed
...
That slice should extend the same lifecycle lane with bounded protocol
initialization and shutdown only. It must keep source-content transfer,
