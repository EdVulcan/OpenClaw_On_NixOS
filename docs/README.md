...
| [OPENCLAW_DBUS_NATIVE_SYSTEMD_CONTROL_PLAN.md](./plans/OPENCLAW_DBUS_NATIVE_SYSTEMD_CONTROL_PLAN.md) | Completed Phase B fixed-unit slice plus live read-only `Unit.After` dependency evidence; native inventory, approved Polkit/D-Bus system-sense restart, and kernel peer-identity verification have real core and Observer VM proof. |
