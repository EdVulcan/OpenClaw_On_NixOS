...
| Level 4 图形栈内生组件 | 约 15%（compositor 与 headed browser attachment 已部署） | 已有 user-owned、资源受限的 nested compositor 和固定 Wayland socket；现有 AI-owned Nix Firefox 已作为 headed client 接入，并通过 session-manager/Observer、真实 visual frame、lease-bound action 与 GNOME 隔离证明。下一缺口是 compositor-native 只读像素帧；原生输入、投影与接管仍未完成。 |
