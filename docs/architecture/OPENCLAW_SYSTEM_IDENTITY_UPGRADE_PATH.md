...
| Level 2 受信会话组件 | 约 96% | trusted-session 契约、takeover suspend/resume-rebind、独立 user-session sidecar、fail-closed authority disconnect、显式 fresh-lease reconnect、持续 bounded capture、sidecar socket browser input/click/new-tab transport、autonomous task dispatch，以及 session-manager/browser-runtime/core restart recovery 已有；sidecar 自身崩溃后的显式 replacement 证明、login-session 安装和更稳定图形 runtime 尚未落地。 |
