...
| Level 2 受信会话组件 | 约 55% | trusted-session 契约、身份一致性、session-manager helper lease、lease 约束的 browser input/click、takeover suspend/resume-rebind，以及审批后受监管的用户态 sidecar IPC heartbeat/start/stop 已有；heartbeat 丢失后的 fail-closed recovery、sidecar-owned capture/action 和更稳定 capture runtime 尚未落地。 |
