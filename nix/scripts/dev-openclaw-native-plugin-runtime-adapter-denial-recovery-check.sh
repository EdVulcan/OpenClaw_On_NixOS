...
FIXTURE_DIR="$REPO_ROOT/.artifacts/openclaw-native-plugin-runtime-adapter-denial-recovery-fixture"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-native-plugin-runtime-adapter-denial-recovery-check.json}"
export OPENCLAW_EVENT_LOG_FILE="${OPENCLAW_EVENT_LOG_FILE:-$REPO_ROOT/.artifacts/openclaw-native-plugin-runtime-adapter-denial-recovery-check-events.jsonl}"
...
{
  "name": "openclaw",
  "version": "0.0.0-runtime-adapter-denial-recovery-fixture",
  "private": true,
  "scripts": {
    "build": "echo RUNTIME_ADAPTER_DENIAL_RECOVERY_ROOT_SECRET_BUILD_BODY"
  }
}
JSON
...
cat > "$PLUGIN_SDK_DIR/package.json" <<'JSON'
{
  ...
  "scripts": {
    "build": "echo RUNTIME_ADAPTER_DENIAL_RECOVERY_SDK_SECRET_BUILD_BODY"
  },
  "dependencies": {
    "zod": "999.0.0-runtime-adapter-denial-recovery-secret-version"
  }
}
JSON
cat > "$PLUGIN_SDK_DIR/src/index.ts" <<'TS'
export const RUNTIME_ADAPTER_DENIAL_RECOVERY_SDK_SECRET_SOURCE_CONTENT = "must-not-leak";
TS
...
if (!taskResponse.ok || taskResponse.registry !== "openclaw-native-plugin-runtime-adapter-task-v0") {
  throw new Error(`native adapter task response mismatch before denial: ${JSON.stringify(taskResponse)}`);
}
if (taskResponse.task?.status !== "queued" || taskResponse.task?.type !== "native_plugin_runtime_adapter_implementation") {
  throw new Error(`new native adapter task should be queued: ${JSON.stringify(taskResponse.task)}`);
}
if (!blocked.ok || blocked.ran !== false || blocked.blocked !== true || blocked.reason !== "policy_requires_approval") {
  throw new Error(`operator should block native adapter task before approval: ${JSON.stringify(blocked)}`);
}
if (!blocked.approval?.id || blocked.approval.id !== taskResponse.approval?.id) {
  throw new Error(`blocked native adapter step should expose linked approval: ${JSON.stringify(blocked.approval)}`);
}
process.stdout.write(blocked.approval.id);
EOF
)"

post_json "$CORE_URL/approvals/$approval_id/deny" '{"deniedBy":"dev-openclaw-native-plugin-runtime-adapter-denial-recovery-check","reason":"deny native plugin runtime adapter before recovery"}' > "$DENIED_FILE"
...
if (!denied.ok || denied.approval?.status !== "denied" || denied.task?.status !== "failed") {
  throw new Error(`denying native adapter approval should fail task: ${JSON.stringify(denied)}`);
}
if (denied.task?.type !== "native_plugin_runtime_adapter_implementation" || denied.task?.plan?.strategy !== "native-plugin-runtime-adapter-v0") {
  throw new Error(`denied native adapter task should retain runtime adapter plan: ${JSON.stringify(denied.task)}`);
}
if (denied.task?.restorable !== true) {
  throw new Error(`denied native adapter task should be restorable: ${JSON.stringify(denied.task)}`);
}
if (denied.task?.policy?.request?.approved === true || denied.task?.policy?.decision?.approved === true) {
  throw new Error(`denied native adapter task must not become approved: ${JSON.stringify(denied.task.policy)}`);
}
...
if (!recovered.ok || recovered.task?.status !== "queued") {
  throw new Error(`recovering denied native adapter task should create queued task: ${JSON.stringify(recovered)}`);
}
if (recovered.task?.recovery?.recoveredFromTaskId !== denied.task?.id) {
  throw new Error(`recovered native adapter task should link to denied source: ${JSON.stringify(recovered.task?.recovery)}`);
}
if (recovered.recoveredFromTask?.recoveredByTaskId !== recovered.task?.id) {
  throw new Error(`denied native adapter task should link to recovered task: ${JSON.stringify(recovered.recoveredFromTask)}`);
}
if (recovered.task?.type !== "native_plugin_runtime_adapter_implementation" || recovered.task?.plan?.strategy !== "native-plugin-runtime-adapter-v0") {
  throw new Error(`recovered native adapter task should preserve runtime adapter plan: ${JSON.stringify(recovered.task)}`);
}
if (recovered.task?.policy?.request?.approved === true || recovered.task?.policy?.decision?.approved === true) {
  throw new Error(`recovered native adapter task must not inherit approval: ${JSON.stringify(recovered.task?.policy)}`);
}
if (recovered.task?.policy?.decision?.decision !== "require_approval") {
  throw new Error(`recovered native adapter task should require fresh approval: ${JSON.stringify(recovered.task?.policy)}`);
}
if (recovered.task?.approval?.required !== true || recovered.task?.approval?.status !== "pending") {
  throw new Error(`recovered native adapter task should have fresh pending approval: ${JSON.stringify(recovered.task?.approval)}`);
}
if (recovered.task?.approval?.requestId === denied.approval?.id) {
  throw new Error(`recovered native adapter task should not reuse denied approval id: ${JSON.stringify(recovered.task?.approval)}`);
}
if (duplicate.ok !== false || duplicate.recoveredByTaskId !== recovered.task?.id) {
  throw new Error(`duplicate native adapter recovery should be rejected: ${JSON.stringify(duplicate)}`);
}
...
post_json "$CORE_URL/approvals/$recovered_approval_id/approve" '{"approvedBy":"dev-openclaw-native-plugin-runtime-adapter-denial-recovery-check","reason":"approve recovered native plugin runtime adapter"}' > "$APPROVED_FILE"
...
if (!recoveredBlocked.ok || recoveredBlocked.ran !== false || recoveredBlocked.blocked !== true || recoveredBlocked.reason !== "policy_requires_approval") {
  throw new Error(`operator should block recovered native adapter task before fresh approval: ${JSON.stringify(recoveredBlocked)}`);
}
if (approved.approval?.status !== "approved" || approved.task?.policy?.decision?.decision !== "audit_only") {
  throw new Error(`fresh native adapter approval should convert task to audited deferred implementation: ${JSON.stringify(approved)}`);
}
if (!deferred.ok || deferred.ran !== false || deferred.blocked !== true || deferred.reason !== "native_plugin_runtime_adapter_implementation_deferred") {
  throw new Error(`approved native adapter recovery should remain implementation-deferred: ${JSON.stringify(deferred)}`);
}
if (deferred.task?.id !== recovered.task?.id || deferred.task?.status !== "queued" || deferred.task?.executionPhase !== "runtime_adapter_implementation_deferred") {
  throw new Error(`deferred native adapter task should stay queued for future implementation: ${JSON.stringify(deferred.task)}`);
}
if (history.summary?.total !== 0 || history.summary?.invoked !== 0 || history.summary?.blocked !== 0) {
  throw new Error(`native adapter denial recovery must not invoke capabilities: ${JSON.stringify(history.summary)}`);
}
...
if (tasks.summary?.counts?.failed !== 1 || tasks.summary?.counts?.queued !== 1 || tasks.summary?.counts?.recoverable !== 1) {
  throw new Error(`task summary should show failed source plus deferred recovered native adapter task: ${JSON.stringify(tasks.summary)}`);
}
...
for (const secret of [
  "RUNTIME_ADAPTER_DENIAL_RECOVERY_ROOT_SECRET_BUILD_BODY",
  "RUNTIME_ADAPTER_DENIAL_RECOVERY_SDK_SECRET_BUILD_BODY",
  "RUNTIME_ADAPTER_DENIAL_RECOVERY_SDK_SECRET_SOURCE_CONTENT",
  "999.0.0-runtime-adapter-denial-recovery-secret-version",
  "0.0.0-runtime-adapter-denial-recovery-fixture",
]) {
  if (raw.includes(secret)) {
    throw new Error(`native adapter denial recovery leaked source, script, dependency, or package detail: ${secret}`);
  }
}

console.log(JSON.stringify({
  openclawNativePluginRuntimeAdapterDenialRecovery: {
