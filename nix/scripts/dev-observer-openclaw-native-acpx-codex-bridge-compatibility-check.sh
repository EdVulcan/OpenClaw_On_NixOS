...
node - <<'EOF' "$HTML_FILE" "$CLIENT_FILE" "$FIRST_FILE" "$SECOND_FILE" "$BRIDGE_FILE" "$DRAFT_FILE" "$APPROVALS_FILE" "$HISTORY_FILE"
...
const raw = JSON.stringify({ html, client, first, second, bridge, draft, approvals, history });
