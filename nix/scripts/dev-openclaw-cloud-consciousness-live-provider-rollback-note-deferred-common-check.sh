...
OBSERVER_CHECK="${PHASE50_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE50_PORT_BASE:-19200}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_50_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-50-rollback-note-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-50-rollback-note-deferred-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase50-prereq"
cleanup() {
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessApprovedRollbackNoteDeferred: { status: "passed", registry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-50-check","reason":"Approve deferred rollback note shell."}' > "$APPROVED_FILE"
...
for (const token of [
  "openclaw-cloud-consciousness-approved-live-provider-rollback-note-deferred",
  "rollback_note_deferred_after_approval",
  "rollback execution",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 50 plan doc missing ${token}`);
}
...
if (!step.ok || shell?.registry !== registry || shell?.implementationStatus !== "deferred_after_approval") {
  throw new Error(`Approved rollback note task should defer after approval: ${JSON.stringify(step)}`);
}
if (!step.task?.phaseHistory?.some((entry) => entry.phase === "cloud_consciousness_live_provider_rollback_note_deferred")) {
  throw new Error(`Task should retain deferred rollback note phase history: ${JSON.stringify(step.task?.phaseHistory)}`);
}
if (
  ...
  throw new Error(`Approved rollback note task must not enable rollback or live provider activity: ${JSON.stringify(shell)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessApprovedRollbackNoteDeferred: { status: "passed", taskId: step.task.id, registry } }, null, 2));
EOF
