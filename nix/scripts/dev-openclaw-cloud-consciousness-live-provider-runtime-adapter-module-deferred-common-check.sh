...
OBSERVER_CHECK="${PHASE26_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE26_PORT_BASE:-8620}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_26_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-26-runtime-adapter-module-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-26-runtime-adapter-module-deferred-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase26-prereq"
cleanup() {
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessApprovedRuntimeAdapterModuleDeferred: { status: "passed", registry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-26-check","reason":"Approve deferred runtime adapter module shell."}' > "$APPROVED_FILE"
...
for (const token of [
  "openclaw-cloud-consciousness-approved-live-provider-runtime-adapter-module-deferred",
  "deferred_after_approval",
  "no source mutation",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 26 plan doc missing ${token}`);
}
...
if (!step.ok || shell?.registry !== registry || shell?.implementationStatus !== "deferred_after_approval") {
  throw new Error(`Approved runtime adapter module should defer after approval: ${JSON.stringify(step)}`);
}
if (!step.task?.phaseHistory?.some((entry) => entry.phase === "cloud_consciousness_live_provider_runtime_adapter_module_deferred")) {
  throw new Error(`Task should retain deferred runtime adapter module phase history: ${JSON.stringify(step.task?.phaseHistory)}`);
}
if (
  ...
  throw new Error(`Approved runtime adapter module must not enable mutation or live activity: ${JSON.stringify(shell)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessApprovedRuntimeAdapterModuleDeferred: { status: "passed", taskId: step.task.id, registry } }, null, 2));
EOF
