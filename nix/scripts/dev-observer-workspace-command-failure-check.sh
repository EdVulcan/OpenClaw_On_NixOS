...
FIXTURE_DIR="$REPO_ROOT/.artifacts/observer-workspace-command-failure-fixture"
WORKSPACE_DIR="$FIXTURE_DIR/openclaw"

export OPENCLAW_CORE_PORT="${OPENCLAW_CORE_PORT:-8850}"
export OPENCLAW_EVENT_HUB_PORT="${OPENCLAW_EVENT_HUB_PORT:-8851}"
export OPENCLAW_SESSION_MANAGER_PORT="${OPENCLAW_SESSION_MANAGER_PORT:-8852}"
export OPENCLAW_BROWSER_RUNTIME_PORT="${OPENCLAW_BROWSER_RUNTIME_PORT:-8853}"
export OPENCLAW_SCREEN_SENSE_PORT="${OPENCLAW_SCREEN_SENSE_PORT:-8854}"
export OPENCLAW_SCREEN_ACT_PORT="${OPENCLAW_SCREEN_ACT_PORT:-8855}"
export OPENCLAW_SYSTEM_SENSE_PORT="${OPENCLAW_SYSTEM_SENSE_PORT:-8856}"
export OPENCLAW_SYSTEM_HEAL_PORT="${OPENCLAW_SYSTEM_HEAL_PORT:-8857}"
export OBSERVER_UI_PORT="${OBSERVER_UI_PORT:-8920}"
export OPENCLAW_WORKSPACE_ROOTS="$WORKSPACE_DIR"
export OPENCLAW_SYSTEM_ALLOWED_ROOTS="$WORKSPACE_DIR"
export OPENCLAW_SYSTEM_COMMAND_ALLOWLIST="npm"
export OPENCLAW_SYSTEM_COMMAND_TIMEOUT_MS="15000"
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-observer-workspace-command-failure-check.json}"
export OPENCLAW_EVENT_LOG_FILE="${OPENCLAW_EVENT_LOG_FILE:-$REPO_ROOT/.artifacts/observer-workspace-command-failure-check-events.jsonl}"
...
{
  "name": "openclaw",
  "version": "0.0.0-observer-command-failure-fixture",
  "private": true,
  "scripts": {
    "typecheck": "node -e \"process.stderr.write('observer-workspace-fail'); process.exit(7)\""
  },
  "workspaces": [
    "apps/*",
    "packages/*"
...
if (!blockedStep.ok || blockedStep.ran !== false || blockedStep.blocked !== true || blockedStep.reason !== "policy_requires_approval") {
  throw new Error(`operator should block before observer workspace command failure approval: ${JSON.stringify(blockedStep)}`);
}
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"dev-observer-workspace-command-failure-check","reason":"approve failing observer fixture workspace command"}' > "$APPROVED_FILE"
...
if (approved.approval?.status !== "approved" || approved.task?.policy?.decision?.decision !== "audit_only") {
  throw new Error(`Observer approval flow should convert failing task to audited execution: ${JSON.stringify(approved)}`);
}
if (!failStep.ok || failStep.ran !== true || failStep.blocked !== false || failStep.task?.status !== "failed") {
  throw new Error(`Observer workspace command failure should fail the task after approval: ${JSON.stringify(failStep)}`);
}
if (!String(failStep.task?.outcome?.reason ?? "").includes("exit code 7")) {
  throw new Error(`Observer failed task should explain exit code 7: ${JSON.stringify(failStep.task?.outcome)}`);
}
...
if (
  ...
  || !String(invocation.summary?.stderr ?? "").includes("observer-workspace-fail")
) {
  throw new Error(`Observer failing workspace command invocation mismatch: ${JSON.stringify(invocation)}`);
}
...
if (
  ...
  || !String(transcript.stderr ?? "").includes("observer-workspace-fail")
) {
  throw new Error(`Observer failing workspace command transcript mismatch: ${JSON.stringify(transcript)}`);
}
if (commandsAfter.summary?.total !== (commandsBefore.summary?.total ?? 0) + 1 || commandsAfter.summary?.failed !== 1) {
  throw new Error(`Observer workspace command failure should add one failed command transcript: ${JSON.stringify({ before: commandsBefore.summary, after: commandsAfter.summary })}`);
}
if (transcripts.items?.[0]?.taskId !== taskResponse.task?.id || transcripts.items?.[0]?.command !== "npm" || transcripts.items?.[0]?.state !== "failed") {
  throw new Error(`Observer command ledger should reference failed workspace command task: ${JSON.stringify(transcripts.items?.[0])}`);
}
if (history.summary?.total !== 1 || history.summary?.invoked !== 1 || history.summary?.blocked !== 0) {
  throw new Error(`Observer capability history should record one failed command invocation attempt: ${JSON.stringify(history.summary)}`);
}
if (history.items?.[0]?.request?.cwd !== workspaceDir || history.items?.[0]?.request?.command !== "npm") {
  throw new Error(`Observer capability history should preserve failed command metadata: ${JSON.stringify(history.items?.[0]?.request)}`);
}
if ((approvals.items ?? []).some((approval) => approval.status === "pending")) {
  throw new Error(`Observer failed workspace command should leave no pending approvals: ${JSON.stringify(approvals.items)}`);
}
...
if (eventTypes.has("task.completed")) {
  throw new Error(`Observer failed workspace command should not emit task.completed: ${JSON.stringify([...eventTypes])}`);
}

console.log(JSON.stringify({
  observerWorkspaceCommandFailure: {
