...
OBSERVER_CHECK="${PHASE69_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE69_PORT_BASE:-21800}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_69_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-69-credential-value-read-task-shell-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-69-credential-value-read-task-shell-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase69-prereq"
...
const html = fs.readFileSync(process.argv[13], "utf8");
const client = fs.readFileSync(process.argv[14], "utf8");
for (const token of [
  "Cloud Consciousness Live Provider Credential Value Read Task Shell",
  "cloud-consciousness-live-provider-credential-value-read-task-shell-panel",
]) {
...
for (const token of [
  ...
  "openclaw-cloud-consciousness-live-provider-credential-value-read-task-shell",
  taskRegistry,
  preflightRegistry,
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-69-check","reason":"Approve real launch task shell before execution preflight and credential value read task shell."}' > "$APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$egress_approval_id/approve" '{"approvedBy":"phase-69-check","reason":"Approve credential value read task shell while keeping endpoint contact and network egress deferred."}' > "$EGRESS_APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$credential_auth_approval_id/approve" '{"approvedBy":"phase-69-check","reason":"Approve credential value read task shell while keeping credential values unread."}' > "$CREDENTIAL_AUTH_APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$credential_read_approval_id/approve" '{"approvedBy":"phase-69-check","reason":"Approve credential value read task shell while keeping the credential value unread."}' > "$CREDENTIAL_READ_APPROVED_FILE"
post_json "$CORE_URL/operator/step" '{}' > "$CREDENTIAL_READ_STEP_FILE"

node - <<'EOF' "$TASK_REGISTRY" "$PREFLIGHT_REGISTRY" "$CREDENTIAL_GATE_REGISTRY" "$ENDPOINT_GATE_REGISTRY" "$ROUTE_PREFLIGHT_REGISTRY" "$EGRESS_TASK_REGISTRY" "$CREDENTIAL_AUTH_ROUTE_REGISTRY" "$CREDENTIAL_AUTH_TASK_REGISTRY" "$CREDENTIAL_AUTH_APPROVED_DEFERRED_REGISTRY" "$CREDENTIAL_READINESS_PREFLIGHT_REGISTRY" "$CREDENTIAL_READ_TASK_REGISTRY" "$PLAN_DOC" "$BEFORE_FILE" "$PREFLIGHT_FILE" "$CREDENTIAL_GATE_FILE" "$ENDPOINT_GATE_FILE" "$ROUTE_PREFLIGHT_FILE" "$EGRESS_TASK_FILE" "$EGRESS_STEP_FILE" "$APPROVED_DEFERRED_FILE" "$CREDENTIAL_AUTH_ROUTE_FILE" "$CREDENTIAL_AUTH_TASK_FILE" "$CREDENTIAL_AUTH_STEP_FILE" "$CREDENTIAL_AUTH_APPROVED_DEFERRED_FILE" "$CREDENTIAL_READINESS_PREFLIGHT_FILE" "$CREDENTIAL_READ_TASK_FILE" "$CREDENTIAL_READ_STEP_FILE"
const fs = require("node:fs");
const taskRegistry = process.argv[2];
const preflightRegistry = process.argv[3];
const credentialGateRegistry = process.argv[4];
...
const credentialReadinessPreflightRegistry = process.argv[11];
const credentialReadTaskRegistry = process.argv[12];
const doc = fs.readFileSync(process.argv[13], "utf8");
const before = JSON.parse(fs.readFileSync(process.argv[14], "utf8"));
const preflightRecord = JSON.parse(fs.readFileSync(process.argv[15], "utf8"));
const credentialGateRecord = JSON.parse(fs.readFileSync(process.argv[16], "utf8"));
const endpointGateRecord = JSON.parse(fs.readFileSync(process.argv[17], "utf8"));
const routePreflightRecord = JSON.parse(fs.readFileSync(process.argv[18], "utf8"));
const egressTaskRecord = JSON.parse(fs.readFileSync(process.argv[19], "utf8"));
const egressStepRecord = JSON.parse(fs.readFileSync(process.argv[20], "utf8"));
const approvedDeferred = JSON.parse(fs.readFileSync(process.argv[21], "utf8"));
const credentialAuthRoute = JSON.parse(fs.readFileSync(process.argv[22], "utf8"));
const credentialAuthTaskRecord = JSON.parse(fs.readFileSync(process.argv[23], "utf8"));
const credentialAuthStepRecord = JSON.parse(fs.readFileSync(process.argv[24], "utf8"));
const credentialAuthApprovedDeferred = JSON.parse(fs.readFileSync(process.argv[25], "utf8"));
const credentialReadinessPreflight = JSON.parse(fs.readFileSync(process.argv[26], "utf8"));
const credentialReadTask = JSON.parse(fs.readFileSync(process.argv[27], "utf8"));
const credentialReadStep = JSON.parse(fs.readFileSync(process.argv[28], "utf8"));
for (const token of [
  "openclaw-cloud-consciousness-live-provider-credential-value-read-task-shell",
  "Requires Phase 68 credential value readiness preflight evidence",
  "credentialValueReadTaskApproved: true",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 69 plan doc missing ${token}`);
}
...
if (
  ...
  throw new Error(`Phase 66 credential value read task shell must not read credentials or perform egress: ${JSON.stringify(credentialAuthStepRecord)}`);
}
if (
...
if (
  ...
  throw new Error(`Phase 69 should create and approve deferred credential value read task shell evidence: ${JSON.stringify({ credentialReadTask, credentialReadStep })}`);
}
if (
...
console.log(JSON.stringify({ openclawCloudConsciousnessCredentialValueReadTaskShell: { status: "passed", taskId: credentialReadTask.task.id, completedTaskId: credentialReadStep.task.id, sourceTaskId: readTaskShell.sourceTaskId, taskRegistry, preflightRegistry, credentialGateRegistry, endpointGateRegistry, routePreflightRegistry, egressTaskRegistry, credentialAuthRouteRegistry, credentialAuthTaskRegistry, credentialAuthApprovedDeferredRegistry, credentialReadinessPreflightRegistry, credentialReadTaskRegistry } }, null, 2));
EOF
