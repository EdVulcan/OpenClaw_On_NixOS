...
FIXTURE_DIR="$REPO_ROOT/.artifacts/openclaw-workspace-command-denial-fixture"
WORKSPACE_DIR="$FIXTURE_DIR/openclaw"

export OPENCLAW_CORE_PORT="${OPENCLAW_CORE_PORT:-8890}"
export OPENCLAW_EVENT_HUB_PORT="${OPENCLAW_EVENT_HUB_PORT:-8891}"
export OPENCLAW_SESSION_MANAGER_PORT="${OPENCLAW_SESSION_MANAGER_PORT:-8892}"
export OPENCLAW_BROWSER_RUNTIME_PORT="${OPENCLAW_BROWSER_RUNTIME_PORT:-8893}"
export OPENCLAW_SCREEN_SENSE_PORT="${OPENCLAW_SCREEN_SENSE_PORT:-8894}"
export OPENCLAW_SCREEN_ACT_PORT="${OPENCLAW_SCREEN_ACT_PORT:-8895}"
export OPENCLAW_SYSTEM_SENSE_PORT="${OPENCLAW_SYSTEM_SENSE_PORT:-8896}"
export OPENCLAW_SYSTEM_HEAL_PORT="${OPENCLAW_SYSTEM_HEAL_PORT:-8897}"
export OBSERVER_UI_PORT="${OBSERVER_UI_PORT:-8960}"
export OPENCLAW_WORKSPACE_ROOTS="$WORKSPACE_DIR"
export OPENCLAW_SYSTEM_ALLOWED_ROOTS="$WORKSPACE_DIR"
export OPENCLAW_SYSTEM_COMMAND_ALLOWLIST="npm"
export OPENCLAW_SYSTEM_COMMAND_TIMEOUT_MS="15000"
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-workspace-command-denial-check.json}"
export OPENCLAW_EVENT_LOG_FILE="${OPENCLAW_EVENT_LOG_FILE:-$REPO_ROOT/.artifacts/openclaw-workspace-command-denial-check-events.jsonl}"
...
{
  "name": "openclaw",
  "version": "0.0.0-command-denial-fixture",
  "private": true,
  "scripts": {
    "typecheck": "printf workspace-command-denial-should-not-run"
  },
  "workspaces": [
    "apps/*",
    "packages/*"
...
if (taskResponse.task?.policy?.decision?.decision !== "require_approval") {
  throw new Error(`workspace command task should start with require_approval: ${JSON.stringify(taskResponse.task?.policy)}`);
}
...
if (commandsBefore.summary?.total !== 0) {
  throw new Error(`denial fixture should start without command transcripts: ${JSON.stringify(commandsBefore.summary)}`);
}
...
post_json "$CORE_URL/approvals/$approval_id/deny" '{"deniedBy":"dev-openclaw-workspace-command-denial-check","reason":"deny fixture workspace command"}' > "$DENIED_FILE"
...
const afterDenyStep = readJson(5);
const tasks = readJson(6);
const latestFailed = readJson(7);
...
if (denied.task?.approval?.required !== false || denied.task?.approval?.status !== "denied") {
  throw new Error(`denied workspace command should resolve approval gate: ${JSON.stringify(denied.task?.approval)}`);
}
...
if (afterDenyStep.ok !== true || afterDenyStep.ran !== false || afterDenyStep.blocked !== false) {
  throw new Error(`operator should have no denied workspace command to execute: ${JSON.stringify(afterDenyStep)}`);
}
if (latestFailed.task?.id !== denied.task?.id || latestFailed.task?.restorable !== true) {
  throw new Error(`latest failed should expose denied workspace command as recoverable history: ${JSON.stringify(latestFailed)}`);
}
if (tasks.summary?.counts?.failed !== 1 || tasks.summary?.counts?.active !== 0 || tasks.summary?.counts?.queued !== 0 || tasks.summary?.counts?.recoverable !== 1) {
  throw new Error(`task summary should show one failed recoverable denied workspace command and no active work: ${JSON.stringify(tasks.summary)}`);
}
if (commandsAfter.summary?.total !== 0 || commandsAfter.summary?.failed !== 0 || commandsAfter.summary?.executed !== 0) {
  throw new Error(`denied workspace command must not create command transcripts: ${JSON.stringify(commandsAfter.summary)}`);
}
if ((transcripts.items ?? []).length !== 0 || transcripts.summary?.total !== 0) {
  throw new Error(`command transcript ledger should remain empty after denial: ${JSON.stringify(transcripts)}`);
}
if (history.summary?.total !== 0 || history.summary?.invoked !== 0 || history.summary?.blocked !== 0) {
  throw new Error(`denied workspace command must not invoke command capability: ${JSON.stringify(history.summary)}`);
}
if (approvals.summary?.counts?.denied !== 1 || approvals.summary?.counts?.pending !== 0 || approvals.summary?.counts?.approved !== 0) {
  throw new Error(`approval summary should show one denied approval and no pending approvals: ${JSON.stringify(approvals.summary)}`);
}
if (approvals.items?.[0]?.id !== blockedStep.approval?.id || approvals.items?.[0]?.status !== "denied") {
  throw new Error(`approval inbox should show denied workspace command approval: ${JSON.stringify(approvals.items?.[0])}`);
}
...
for (const forbidden of ["approval.approved", "capability.invoked", "system.command.executed", "task.completed"]) {
  if (eventTypes.has(forbidden)) {
    throw new Error(`denied workspace command should not emit ${forbidden}: ${JSON.stringify([...eventTypes])}`);
  }
}

console.log(JSON.stringify({
  openclawWorkspaceCommandDenial: {
    task: {
      id: denied.task.id,
