...
OBSERVER_CHECK="${PHASE67_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE67_PORT_BASE:-21600}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_67_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-67-credential-value-authorization-approved-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-67-credential-value-authorization-approved-deferred-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase67-prereq"
...
if [[ "$OBSERVER_CHECK" == "true" ]]; then
  ...
  node - <<'EOF' "$TASK_REGISTRY" "$PREFLIGHT_REGISTRY" "$CREDENTIAL_GATE_REGISTRY" "$ENDPOINT_GATE_REGISTRY" "$ROUTE_PREFLIGHT_REGISTRY" "$EGRESS_TASK_REGISTRY" "$CREDENTIAL_AUTH_ROUTE_REGISTRY" "$CREDENTIAL_AUTH_TASK_REGISTRY" "$CREDENTIAL_AUTH_APPROVED_DEFERRED_REGISTRY" "$HTML_FILE" "$CLIENT_FILE"
...
const html = fs.readFileSync(process.argv[11], "utf8");
const client = fs.readFileSync(process.argv[12], "utf8");
for (const token of [
  "Cloud Consciousness Live Provider Credential Value Authorization Approved Deferred",
  "cloud-consciousness-live-provider-credential-value-authorization-approved-deferred-panel",
]) {
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessCredentialValueAuthorizationApprovedDeferred: { status: "passed", taskRegistry, preflightRegistry, credentialGateRegistry, endpointGateRegistry, routePreflightRegistry, egressTaskRegistry, credentialAuthRouteRegistry, credentialAuthTaskRegistry, credentialAuthApprovedDeferredRegistry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-67-check","reason":"Approve real launch task shell before execution preflight and credential value authorization approved deferred."}' > "$APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$egress_approval_id/approve" '{"approvedBy":"phase-67-check","reason":"Approve credential value authorization approved deferred while keeping endpoint contact and network egress deferred."}' > "$EGRESS_APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$credential_auth_approval_id/approve" '{"approvedBy":"phase-67-check","reason":"Approve credential value authorization approved deferred while keeping credential values unread."}' > "$CREDENTIAL_AUTH_APPROVED_FILE"
...
const credentialAuthApprovedDeferred = JSON.parse(fs.readFileSync(process.argv[23], "utf8"));
for (const token of [
  ...
  if (!doc.includes(token)) throw new Error(`Phase 67 plan doc missing ${token}`);
}
...
if (
  ...
  throw new Error(`Phase 66 credential value authorization approved deferred must not read credentials or perform egress: ${JSON.stringify(credentialAuthStepRecord)}`);
}
if (
  ...
  || credentialAuthApprovedDeferred.summary?.credentialValueAuthorizationTaskCreated !== true
  ...
  || credentialAuthApprovedDeferred.summary?.credentialValueAccessAuthorized !== false
  || credentialAuthApprovedDeferred.summary?.credentialValueAccessDenied !== true
  || credentialAuthApprovedDeferred.summary?.credentialValueIncluded !== false
  || credentialAuthApprovedDeferred.summary?.credentialValueRead !== false
  || credentialAuthApprovedDeferred.summary?.credentialValueExposed !== false
  || credentialAuthApprovedDeferred.summary?.providerCredentialRead !== false
  || credentialAuthApprovedDeferred.summary?.endpointContacted !== false
  || credentialAuthApprovedDeferred.summary?.networkEgress !== false
  || credentialAuthApprovedDeferred.summary?.liveProviderCallEnabled !== false
  ...
  throw new Error(`Phase 67 should expose approved deferred credential value authorization evidence without credential reads or egress: ${JSON.stringify(credentialAuthApprovedDeferred)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessCredentialValueAuthorizationApprovedDeferred: { status: "passed", taskId: credentialAuthTaskRecord.task.id, sourceTaskId: credentialAuthApprovedDeferred.summary.sourceTaskId, taskRegistry, preflightRegistry, credentialGateRegistry, endpointGateRegistry, routePreflightRegistry, egressTaskRegistry, credentialAuthRouteRegistry, credentialAuthTaskRegistry, credentialAuthApprovedDeferredRegistry } }, null, 2));
EOF
