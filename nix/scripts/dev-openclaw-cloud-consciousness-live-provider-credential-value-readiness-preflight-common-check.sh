...
OBSERVER_CHECK="${PHASE68_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE68_PORT_BASE:-21700}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_68_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-68-credential-value-readiness-preflight-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-68-credential-value-readiness-preflight-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase68-prereq"
...
const html = fs.readFileSync(process.argv[12], "utf8");
const client = fs.readFileSync(process.argv[13], "utf8");
for (const token of [
  "Cloud Consciousness Live Provider Credential Value Readiness Preflight",
  "cloud-consciousness-live-provider-credential-value-readiness-preflight-panel",
]) {
...
for (const token of [
  ...
  "openclaw-cloud-consciousness-live-provider-credential-value-readiness-preflight",
  taskRegistry,
  preflightRegistry,
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessCredentialValueReadinessPreflight: { status: "passed", taskRegistry, preflightRegistry, credentialGateRegistry, endpointGateRegistry, routePreflightRegistry, egressTaskRegistry, credentialAuthRouteRegistry, credentialAuthTaskRegistry, credentialAuthApprovedDeferredRegistry, credentialReadinessPreflightRegistry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-68-check","reason":"Approve real launch task shell before execution preflight and credential value readiness preflight."}' > "$APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$egress_approval_id/approve" '{"approvedBy":"phase-68-check","reason":"Approve credential value readiness preflight while keeping endpoint contact and network egress deferred."}' > "$EGRESS_APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$credential_auth_approval_id/approve" '{"approvedBy":"phase-68-check","reason":"Approve credential value readiness preflight while keeping credential values unread."}' > "$CREDENTIAL_AUTH_APPROVED_FILE"
...
const credentialAuthApprovedDeferredRegistry = process.argv[10];
const credentialReadinessPreflightRegistry = process.argv[11];
const doc = fs.readFileSync(process.argv[12], "utf8");
const before = JSON.parse(fs.readFileSync(process.argv[13], "utf8"));
const preflightRecord = JSON.parse(fs.readFileSync(process.argv[14], "utf8"));
const credentialGateRecord = JSON.parse(fs.readFileSync(process.argv[15], "utf8"));
const endpointGateRecord = JSON.parse(fs.readFileSync(process.argv[16], "utf8"));
const routePreflightRecord = JSON.parse(fs.readFileSync(process.argv[17], "utf8"));
const egressTaskRecord = JSON.parse(fs.readFileSync(process.argv[18], "utf8"));
const egressStepRecord = JSON.parse(fs.readFileSync(process.argv[19], "utf8"));
const approvedDeferred = JSON.parse(fs.readFileSync(process.argv[20], "utf8"));
const credentialAuthRoute = JSON.parse(fs.readFileSync(process.argv[21], "utf8"));
const credentialAuthTaskRecord = JSON.parse(fs.readFileSync(process.argv[22], "utf8"));
const credentialAuthStepRecord = JSON.parse(fs.readFileSync(process.argv[23], "utf8"));
const credentialAuthApprovedDeferred = JSON.parse(fs.readFileSync(process.argv[24], "utf8"));
const credentialReadinessPreflight = JSON.parse(fs.readFileSync(process.argv[25], "utf8"));
for (const token of [
  "openclaw-cloud-consciousness-live-provider-credential-value-readiness-preflight",
  ...
  if (!doc.includes(token)) throw new Error(`Phase 68 plan doc missing ${token}`);
}
...
if (
  ...
  throw new Error(`Phase 66 credential value readiness preflight must not read credentials or perform egress: ${JSON.stringify(credentialAuthStepRecord)}`);
}
if (
  ...
  throw new Error(`Phase 68 prerequisite should expose Phase 67 approved deferred credential value authorization evidence: ${JSON.stringify(credentialAuthApprovedDeferred)}`);
}
const readinessShell = credentialReadinessPreflight.task?.cloudConsciousnessLiveProviderCredentialValueAuthorization;
const readinessPreflight = credentialReadinessPreflight.preflight;
if (
  ...
  || readinessPreflight?.registry !== credentialReadinessPreflightRegistry
  || readinessPreflight?.status !== "credential_value_readiness_preflight_ready_deferred"
  || readinessPreflight?.summary?.ready !== true
  || readinessPreflight?.summary?.credentialValueReadinessPreflightRecorded !== true
  || readinessPreflight?.summary?.credentialValueAuthorizationApprovedDeferredFound !== true
  || readinessPreflight?.summary?.sourceTaskId !== credentialAuthStepRecord.task?.id
  || readinessPreflight?.summary?.credentialValueAccessAuthorized !== false
  || readinessPreflight?.summary?.credentialValueAccessDenied !== true
  || readinessPreflight?.summary?.credentialValueRead !== false
  || readinessPreflight?.summary?.providerCredentialRead !== false
  || readinessPreflight?.summary?.endpointContacted !== false
  || readinessPreflight?.summary?.networkEgress !== false
  || readinessPreflight?.summary?.liveProviderCallEnabled !== false
  || readinessPreflight?.next?.recommendedSlice !== "openclaw-cloud-consciousness-live-provider-credential-value-read-task-shell"
) {
  throw new Error(`Phase 68 should record credential value readiness preflight without credential reads or egress: ${JSON.stringify(credentialReadinessPreflight)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessCredentialValueReadinessPreflight: { status: "passed", taskId: credentialReadinessPreflight.task.id, sourceTaskId: readinessPreflight.summary.sourceTaskId, taskRegistry, preflightRegistry, credentialGateRegistry, endpointGateRegistry, routePreflightRegistry, egressTaskRegistry, credentialAuthRouteRegistry, credentialAuthTaskRegistry, credentialAuthApprovedDeferredRegistry, credentialReadinessPreflightRegistry } }, null, 2));
EOF
