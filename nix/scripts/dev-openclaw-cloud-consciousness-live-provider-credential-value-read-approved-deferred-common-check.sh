...
OBSERVER_CHECK="${PHASE70_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE70_PORT_BASE:-21900}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_70_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-70-credential-value-read-approved-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-70-credential-value-read-approved-deferred-check.json}"
...
const html = fs.readFileSync(process.argv[14], "utf8");
const client = fs.readFileSync(process.argv[15], "utf8");
for (const token of [
  "Cloud Consciousness Live Provider Credential Value Read Approved Deferred",
  "cloud-consciousness-live-provider-credential-value-read-approved-deferred-panel",
]) {
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-70-check","reason":"Approve real launch task shell before execution preflight and credential value read approved deferred."}' > "$APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$egress_approval_id/approve" '{"approvedBy":"phase-70-check","reason":"Approve credential value read approved deferred while keeping endpoint contact and network egress deferred."}' > "$EGRESS_APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$credential_auth_approval_id/approve" '{"approvedBy":"phase-70-check","reason":"Approve credential value read approved deferred while keeping credential values unread."}' > "$CREDENTIAL_AUTH_APPROVED_FILE"
...
post_json "$CORE_URL/approvals/$credential_read_approval_id/approve" '{"approvedBy":"phase-70-check","reason":"Approve credential value read approved deferred while keeping the credential value unread."}' > "$CREDENTIAL_READ_APPROVED_FILE"
...
const credentialReadTaskRegistry = process.argv[12];
const credentialReadApprovedDeferredRegistry = process.argv[13];
const doc = fs.readFileSync(process.argv[14], "utf8");
const before = JSON.parse(fs.readFileSync(process.argv[15], "utf8"));
const preflightRecord = JSON.parse(fs.readFileSync(process.argv[16], "utf8"));
const credentialGateRecord = JSON.parse(fs.readFileSync(process.argv[17], "utf8"));
const endpointGateRecord = JSON.parse(fs.readFileSync(process.argv[18], "utf8"));
const routePreflightRecord = JSON.parse(fs.readFileSync(process.argv[19], "utf8"));
const egressTaskRecord = JSON.parse(fs.readFileSync(process.argv[20], "utf8"));
const egressStepRecord = JSON.parse(fs.readFileSync(process.argv[21], "utf8"));
const approvedDeferred = JSON.parse(fs.readFileSync(process.argv[22], "utf8"));
const credentialAuthRoute = JSON.parse(fs.readFileSync(process.argv[23], "utf8"));
const credentialAuthTaskRecord = JSON.parse(fs.readFileSync(process.argv[24], "utf8"));
const credentialAuthStepRecord = JSON.parse(fs.readFileSync(process.argv[25], "utf8"));
const credentialAuthApprovedDeferred = JSON.parse(fs.readFileSync(process.argv[26], "utf8"));
const credentialReadinessPreflight = JSON.parse(fs.readFileSync(process.argv[27], "utf8"));
const credentialReadTask = JSON.parse(fs.readFileSync(process.argv[28], "utf8"));
const credentialReadStep = JSON.parse(fs.readFileSync(process.argv[29], "utf8"));
const credentialReadApprovedDeferred = JSON.parse(fs.readFileSync(process.argv[30], "utf8"));
for (const token of [
  "openclaw-cloud-consciousness-live-provider-credential-value-read-approved-deferred",
  "Requires Phase 69 approved credential value read task shell evidence",
  "credentialValueReadDeferred: true",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 70 plan doc missing ${token}`);
}
...
if (
  ...
  throw new Error(`Phase 66 credential value read approved deferred must not read credentials or perform egress: ${JSON.stringify(credentialAuthStepRecord)}`);
}
if (
...
if (
  ...
  throw new Error(`Phase 69 should create and approve deferred credential value read approved deferred evidence: ${JSON.stringify({ credentialReadTask, credentialReadStep })}`);
}
if (
...
console.log(JSON.stringify({ openclawCloudConsciousnessCredentialValueReadApprovedDeferred: { status: "passed", taskId: credentialReadTask.task.id, completedTaskId: credentialReadStep.task.id, sourceTaskId: credentialReadApprovedDeferred.summary.sourceTaskId, taskRegistry, preflightRegistry, credentialGateRegistry, endpointGateRegistry, routePreflightRegistry, egressTaskRegistry, credentialAuthRouteRegistry, credentialAuthTaskRegistry, credentialAuthApprovedDeferredRegistry, credentialReadinessPreflightRegistry, credentialReadTaskRegistry, credentialReadApprovedDeferredRegistry } }, null, 2));
EOF
