...
FIXTURE_DIR="$REPO_ROOT/.artifacts/observer-openclaw-native-plugin-runtime-activation-denial-recovery-fixture"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-observer-native-plugin-runtime-activation-denial-recovery-check.json}"
export OPENCLAW_EVENT_LOG_FILE="${OPENCLAW_EVENT_LOG_FILE:-$REPO_ROOT/.artifacts/observer-native-plugin-runtime-activation-denial-recovery-check-events.jsonl}"
...
{
  "name": "openclaw",
  "version": "0.0.0-observer-runtime-activation-denial-recovery-fixture",
  "private": true,
  "scripts": {
    "build": "echo OBSERVER_RUNTIME_ACTIVATION_DENIAL_RECOVERY_ROOT_SECRET_BUILD_BODY"
  }
}
JSON
...
cat > "$PLUGIN_SDK_DIR/package.json" <<'JSON'
{
  ...
  "scripts": {
    "build": "echo OBSERVER_RUNTIME_ACTIVATION_DENIAL_RECOVERY_SDK_SECRET_BUILD_BODY"
  }
}
JSON
cat > "$PLUGIN_SDK_DIR/src/index.ts" <<'TS'
export const OBSERVER_RUNTIME_ACTIVATION_DENIAL_RECOVERY_SDK_SECRET_SOURCE_CONTENT = "must-not-leak";
TS
...
if (!taskResponse.ok || taskResponse.registry !== "openclaw-native-plugin-runtime-activation-task-v0") {
  throw new Error(`observer native activation task response mismatch: ${JSON.stringify(taskResponse)}`);
}
if (!blocked.ok || blocked.blocked !== true || blocked.reason !== "policy_requires_approval") {
  throw new Error(`observer native activation task should block before denial: ${JSON.stringify(blocked)}`);
}
process.stdout.write(taskResponse.approval.id);
EOF
)"

post_json "$CORE_URL/approvals/$approval_id/deny" '{"deniedBy":"dev-observer-openclaw-native-plugin-runtime-activation-denial-recovery-check","reason":"observer denies native runtime activation before recovery"}' > "$DENIED_FILE"
...
if (denied.approval?.status !== "denied" || denied.task?.status !== "failed" || denied.task?.restorable !== true) {
  throw new Error(`observer denied native activation task should fail and become restorable: ${JSON.stringify(denied)}`);
}
...
if (!recovered.ok || recovered.task?.status !== "queued") {
  throw new Error(`observer recovered native activation task should be queued: ${JSON.stringify(recovered)}`);
}
if (recovered.task?.type !== "native_plugin_runtime_activation") {
  throw new Error(`observer recovered task should remain native activation type: ${JSON.stringify(recovered.task)}`);
}
if (recovered.task?.approval?.requestId === denied.approval?.id || recovered.task?.approval?.status !== "pending") {
  throw new Error(`observer recovered native activation should use a fresh pending approval: ${JSON.stringify(recovered.task?.approval)}`);
}
...
post_json "$CORE_URL/approvals/$recovered_approval_id/approve" '{"approvedBy":"dev-observer-openclaw-native-plugin-runtime-activation-denial-recovery-check","reason":"observer approves recovered native runtime activation"}' > "$APPROVED_FILE"
...
if (recoveredBlocked.blocked !== true || recoveredBlocked.reason !== "policy_requires_approval") {
  throw new Error(`observer recovered native activation should block before fresh approval: ${JSON.stringify(recoveredBlocked)}`);
}
if (approved.approval?.status !== "approved" || approved.task?.policy?.decision?.decision !== "audit_only") {
  throw new Error(`observer recovered native activation approval should become audited: ${JSON.stringify(approved)}`);
}
if (deferred.blocked !== true || deferred.reason !== "native_plugin_runtime_activation_deferred") {
  throw new Error(`observer approved native activation recovery should remain deferred: ${JSON.stringify(deferred)}`);
}
if (history.summary?.total !== 0 || history.summary?.invoked !== 0 || history.summary?.blocked !== 0) {
  throw new Error(`observer native activation denial recovery must not invoke capabilities: ${JSON.stringify(history.summary)}`);
}
...
if (tasks.summary?.counts?.failed !== 1 || tasks.summary?.counts?.queued !== 1) {
  throw new Error(`observer tasks should show failed source plus deferred recovered native activation task: ${JSON.stringify(tasks.summary)}`);
}
for (const secret of [
  "OBSERVER_RUNTIME_ACTIVATION_DENIAL_RECOVERY_ROOT_SECRET_BUILD_BODY",
  "OBSERVER_RUNTIME_ACTIVATION_DENIAL_RECOVERY_SDK_SECRET_BUILD_BODY",
  "OBSERVER_RUNTIME_ACTIVATION_DENIAL_RECOVERY_SDK_SECRET_SOURCE_CONTENT",
  "0.0.0-observer-runtime-activation-denial-recovery-fixture",
]) {
  if (raw.includes(secret)) {
    throw new Error(`Observer native activation denial recovery leaked source, script, or package detail: ${secret}`);
  }
}

console.log(JSON.stringify({
  observerOpenClawNativePluginRuntimeActivationDenialRecovery: {
