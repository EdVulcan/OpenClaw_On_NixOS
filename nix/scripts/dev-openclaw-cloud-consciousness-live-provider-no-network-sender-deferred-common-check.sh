...
OBSERVER_CHECK="${PHASE38_OBSERVER_CHECK:-false}"
PORT_BASE="${PHASE38_PORT_BASE:-8860}"
...
PLAN_DOC="$REPO_ROOT/docs/plans/OPENCLAW_PHASE_38_PLAN.md"
...
export OPENCLAW_CORE_STATE_FILE="${OPENCLAW_CORE_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-core-phase-38-no-network-sender-deferred-check.json}"
export OPENCLAW_SYSTEM_HEAL_STATE_FILE="${OPENCLAW_SYSTEM_HEAL_STATE_FILE:-$REPO_ROOT/.artifacts/openclaw-system-heal-phase-38-no-network-sender-deferred-check.json}"
...
seed_live_provider_call_prerequisites "$CLOUD_DIR" "$PROVIDER_RESPONSE_FILE" "$RUNBOOK_FILE" "$EXECUTION_PLAN_FILE" "phase38-prereq"
cleanup() {
...
console.log(JSON.stringify({ observerOpenClawCloudConsciousnessApprovedNoNetworkSenderDeferred: { status: "passed", registry } }, null, 2));
EOF
  exit 0
fi
...
post_json "$CORE_URL/approvals/$approval_id/approve" '{"approvedBy":"phase-38-check","reason":"Approve deferred no-network sender shell."}' > "$APPROVED_FILE"
...
for (const token of [
  "openclaw-cloud-consciousness-approved-live-provider-no-network-sender-deferred",
  "deferred_after_approval",
  "executable provider egress remains disabled",
]) {
  if (!doc.includes(token)) throw new Error(`Phase 38 plan doc missing ${token}`);
}
...
if (!step.ok || shell?.registry !== registry || shell?.implementationStatus !== "deferred_after_approval") {
  throw new Error(`Approved no-network sender task should defer after approval: ${JSON.stringify(step)}`);
}
if (!step.task?.phaseHistory?.some((entry) => entry.phase === "cloud_consciousness_live_provider_no_network_sender_deferred")) {
  throw new Error(`Task should retain deferred no-network sender phase history: ${JSON.stringify(step.task?.phaseHistory)}`);
}
if (
  ...
  throw new Error(`Approved no-network sender task must not enable live provider activity: ${JSON.stringify(shell)}`);
}
console.log(JSON.stringify({ openclawCloudConsciousnessApprovedNoNetworkSenderDeferred: { status: "passed", taskId: step.task.id, registry } }, null, 2));
EOF
