...
import {
  OPENCLAW_NATIVE_PLUGIN_CAPABILITY_DESCRIPTORS,
  OPENCLAW_NATIVE_PLUGIN_SDK_GOVERNANCE_DESCRIPTOR,
  OPENCLAW_NATIVE_PLUGIN_SDK_PLUGIN_DESCRIPTOR,
} from "./plugin-capability-descriptors.mjs";
...
export function createOpenClawNativePluginRegistry({ generatedAt = new Date().toISOString() } = {}) {
  const pluginSdkContract = createOpenClawNativePluginContract({
    plugin: OPENCLAW_NATIVE_PLUGIN_SDK_PLUGIN_DESCRIPTOR,
    governance: OPENCLAW_NATIVE_PLUGIN_SDK_GOVERNANCE_DESCRIPTOR,
    capabilities: OPENCLAW_NATIVE_PLUGIN_CAPABILITY_DESCRIPTORS,
  });
